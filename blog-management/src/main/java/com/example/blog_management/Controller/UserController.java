...
import com.example.blog_management.DTO.ResetPasswordRequest;
import com.example.blog_management.DTO.VerifyOtpRequest;
import com.example.blog_management.DTO.userDTO;
import com.example.blog_management.Model.User;
import com.example.blog_management.Service.EmailService;
...
public class UserController {
    ...
    public String Loginuser(@RequestBody User user){
        ...
        emailService.sendEmail(user.getEmail(),"logged in blog press","you are logged in blog press thanks for experince for our services");
        return "fail";
    }
    @Operation(
    ...
    @Operation(
            summary = "SEND OTP TO USER REST API",
            description = "REST API to FORGOT PASSWORD BY SENDING OTP"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "201",
    ...
    public String forgotPassword(@RequestBody ForgotPasswordDto forgotPasswordDto){
        return userService.sendOtp(forgotPasswordDto);
    }
    @Operation(
            summary = "VERIFY OTP  REST API",
            description = "REST API to VERIFY OTP BY SENDING TO EMAIL"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "201",
    ...
    public String VerifyOtp(@RequestBody VerifyOtpRequest verifyOtpRequest){
        return userService.verifyOtp(verifyOtpRequest.getEmail(), verifyOtpRequest.getOtp());
    }
    @Operation(
            summary = "RESET PASSWORD  REST API",
            description = "REST API to RESET PASSWORD"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "201",
    ...
    public String SetPassword(@RequestBody ResetPasswordRequest resetPasswordRequest){
